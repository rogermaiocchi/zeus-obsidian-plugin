...
class AutoIndexer {
  ...
  async _runPassport(path) {
    ...
    // PassportIndex.findByQuery existe mas rebuildOne não — usa batch-extract
    // com array unitário via httpClient se disponível.
    if (this.plugin.httpClient && typeof this.plugin.httpClient.passportBatchExtract === 'function') {
      try {
        const r = await this.plugin.httpClient.passportBatchExtract([path], []);
        return { passport: path, daemon: !!r };
      } catch (e) {
