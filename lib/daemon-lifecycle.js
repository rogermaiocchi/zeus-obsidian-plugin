...
class DaemonLifecycle {
  constructor(plugin, options = {}) {
    ...
    // v1.5.1 — fix codex MED: mutex para ensureRunning(). Sem isso, duas chamadas
    // concorrentes (ex: onload race + status command paralelo) podem passar pelo
    // health-check inicial e cada uma spawnar seu daemon — colisão na porta 2223.
  ...
  async ensureRunning() {
    // v1.5.1 — fix codex MED: serializa start. Promessa única reaproveitada
    // por chamadas concorrentes até resolver (ou rejeitar).
    if (this._startPromise) return this._startPromise;
    this._startPromise = (async () => {
      try { return await this._doEnsureRunning(); }
  ...
  async stop({ graceMs = 2000 } = {}) {
    ...
    this.child = null;
    this.spawnedByUs = false;
    // v1.5.1 — fix codex LOW: aguarda 'exit' do filho até graceMs. Só envia
    // SIGKILL se SIGTERM não fechou o processo dentro da janela. Garante que
    // shutdown gracioso do NIO/Swift complete antes de matar à força.
    let exited = false;
    ...
    if (!exited) {
      ...
      // pequena janela final para 'exit' depois do SIGKILL — útil para logs corretos.
