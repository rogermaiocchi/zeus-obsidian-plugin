/*
 * embed-ios.js — v1.12 embed-relay + iOS embed schema versionado.
 *
 * Codex audit v1.12 aprovou estratégia híbrida two-tier:
 *
 *   CAMADA 1 (default ON) — Relay Mac via Tailscale/loopback HTTP daemon
 *     iOS chama daemon Mac (qualquer device com NLContextualEmbedding nativa) e
 *     persiste resultado 512-dim em data/embeddings.jsonl (mesmo formato Mac).
 *     Quality: Apple-native pt-BR otimizada. Latência: 100-500ms via Tailscale.
 *     Funciona quando Mac está acessível (Tailscale mesh OR mesma LAN).
 *
 ...
 *       source: 'daemon-mac' | 'daemon-ios-relay' | 'transformers-ios',
...
// =========================================================================
// EmbedRelay — camada 1 (preferida): tenta daemon HTTP via Tailscale/loopback.
// Quando bem-sucedido, persiste no embeddings.jsonl Mac canônico (512-dim).
// =========================================================================
