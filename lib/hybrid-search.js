...
// Bitmask para sources internas (8 retrievers). Permite operações de set
...
let _simhash;
try {
  _simhash = require('./zeus-simhash');
} catch (e) {
  console.warn('[zeus.hybrid] zeus-simhash não carregou — pré-filtro turbo quantico desativado:', e.message);
  _simhash = null;
}
...
class HybridSearch {
  ...
  async query(q, topN = 30, opts = {}) {
    ...
    let _queryVec = null; // capturado para SimHash pré-filtro (turbo quantico)
    try {
      const sem = await this.plugin.searcher.search(q, topN * 2);
      lists.push((sem || []).map(x => ({ path: x.path, source: 'semantic' })));
      // Captura query vec se searcher expuser (para camada SimHash abaixo).
    ...
    // 1b) SimHash turbo quantico — pré-filtro Hamming sobre embeddings.sh.
    //     Só ativo quando: _simhash disponível + queryVec capturado + embeddings têm sh.
    //     Source bit `simhash` marca itens para auditoria no MMR.
    if (_simhash && _queryVec) {
      try {
        const shHits = this._simhashRetriever(_queryVec, topN * 2);
        if (shHits.length > 0) lists.push(shHits);
      } catch (e) { console.warn('[zeus.hybrid] simhash retrieval failed', e.message); }
    }
