...
class HybridSearch {
  ...
  async query(q, topN = 30) {
    ...
    //    em vault-relative. mode propaga via `_lastSpotlightMode` para que UI
    //    surface ao usuário (codex MED: "spotlight" | "stale" | "mdfind-fallback").
    try {
      if (this.plugin.httpClient && this.plugin.vaultRoot) {
        ...
        const root = this.plugin.vaultRoot.endsWith('/')
          ? this.plugin.vaultRoot : this.plugin.vaultRoot + '/';
        const list = [];
        for (const raw of (r.results || [])) {
          if (typeof raw !== 'string') continue;
          let rel = raw;
          if (raw.startsWith(root)) rel = raw.slice(root.length);
          if (rel.startsWith('/')) continue;        // path fora do vault
          if (!rel.endsWith('.md')) continue;        // só notas, por enquanto
