...
class HybridSearch {
  ...
  _bm25Retriever(query, topN, maxCorpus = 2000) {
    ...
    try {
      ...
      // Constrói corpus a partir das notas com embedding (esse é o universo
      // já indexado pelo Zeus). Em vault sem embeddings, BM25 fica vazio.
      const notes = [];
      let count = 0;
  ...
  async sisterNotes(filePath, topN = 12, opts = {}) {
    ...
    //    destino somando weight. Apenas se o grafo estiver carregado.
    try {
