...
// Stems pt-BR — regex aplicado em ordem decrescente de comprimento do sufixo.
// v1.15.0 fix: sufixos mais longos primeiro para evitar over-stem prematuro.
// Exemplo: "universidades" → /idades$/ (6 chars) antes de /s$/ (1 char).
// Bug anterior: /s$/ capturava antes de /idades$/ pois plural estava antes.
//
...
const PT_SUFFIXES = [
  ...
  /(?:idades|edades)$/,         // universidades → univers  (bug fix v1.15.0)
  /(?:amente|mente)$/,          // rapidamente → rapid
  // Sufixos médios (4-5 chars)
  /(?:ções|coes|sões|soes)$/,   // ações/visões plural
  /(?:ável|ível)$/,             // amável → am
  /(?:ção|cao|são|sao)$/,       // ação/visão singular
  /(?:ados|idas|idos|adas)$/,   // particípios plural
  /(?:idade|edade)$/,           // universidade → univers
  /(?:inho|inha)$/,             // diminutivo
