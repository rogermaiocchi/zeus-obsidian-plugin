...
class PassportIndex {
  ...
  async buildOne(filePath, domainOptions = []) {
    ...
    // v1.16.1 conformidade C1: coage passport.path para vault-relative ANTES de
    // persistir. O daemon (Mac) devolve path absoluto /Users/...; persistir
    // absoluto polui passports.jsonl + manifest.json sincronizados por iCloud e
    // quebra cross-device (Mac mini=rogermaiocchi, MacBook=maiocchi, iPad sem /Users).
    // _vaultRelative é idempotente para paths já relativos (caminho iOS-local).
  ...
  async _updateManifestEntry(passport) {
    ...
    // v1.16.1 conformidade C1: chave do manifest SEMPRE vault-relative (defesa
    // em profundidade — mesmo que um caller futuro passe path absoluto).
    ...
    for (let i = 0; i < notes.length; i += BATCH) {
      ...
      try {
        ...
        for (const p of items) {
          if (p && p.path) {
            // v1.16.1 conformidade C1: buildAll também coage para vault-relative
            // (o daemon ecoa absPath /Users/... no batch-extract no Mac).
