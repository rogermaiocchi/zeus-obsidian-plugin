...
class PassportIndex {
  ...
  async buildOne(filePath, domainOptions = []) {
    ...
    try {
      ...
      if (fs.existsSync(abs)) {
        ...
        currentSha = require('crypto').createHash('sha256').update(content).digest('hex');
      }
    } catch (e) {
    ...
    try { this._updateManifestEntry(passport); } catch (e) {
  ...
  _updateManifestEntry(passport) {
    ...
    this.plugin.indexer.saveManifest(m);
  }
