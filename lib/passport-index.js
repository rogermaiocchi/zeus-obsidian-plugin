...
class PassportIndex {
  ...
  async buildOne(filePath, domainOptions = []) {
    ...
    // v1.15 — S8 Check 1: o daemon recebe absPath (Mac) e ecoa passport.path
    // absoluto. Coage para vault-relative ANTES de virar chave do cache/JSONL,
    // garantindo que o key do map case com o .path persistido (sem duplicatas).
    ...
    for (let i = 0; i < notes.length; i += BATCH) {
      ...
      try {
        ...
        for (const p of items) {
          if (p && p.path) {
            // v1.15 — codex round 5 #5: coage path absoluto (daemon Mac) para
            // vault-relative ANTES do map.set, igual buildOne. Sem isso, buildAll
            // criaria chave absoluta no cache + linha relativa no disco (duplicata).
  ...
  async loadAll() {
    ...
    for (const ln of lines) {
      ...
      try {
        ...
        if (obj && obj.path) {
          // v1.15 — S8 Check 1: coage paths absolutos legados (/Users/...) para
          // vault-relative na leitura. Arquivos antigos (pré-v1.15, gravados pelo
          // caminho daemon sem coerção) auto-migram no próximo saveAll. iCloud só
          // deve sincronizar paths relativos, que resolvem em qualquer device.
  ...
  async saveAll(map) {
    ...
    for (const passport of map.values()) {
      // v1.15 — S8 Check 1: nenhum path absoluto entra no passports.jsonl, que
      // sincroniza via iCloud. Coage defensivamente para vault-relative no único
      // chokepoint de escrita, independente do que o daemon ecoou.
  ...
  async findByQueryLocal(query, options = {}) {
    ...
    // Constrói corpus para BM25 sobre one_line_summary (ou basename fallback)
