...
class PassportScheduler {
  ...
  async sweep() {
    ...
    try {
      ...
      const files = this.plugin.indexer.enumerateFiles().filter(f => f.ext === 'md');
      const passports = this.plugin.passport.loadAll();
  ...
  stats() {
    return {
      ...
      coordinator: this.coord ? this.coord.stats() : null,
    };
  }
}
