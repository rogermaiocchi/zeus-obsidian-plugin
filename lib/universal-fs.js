...
function generateDeviceId() {
  // Privacy invariant: the deviceId is written to `extracted_by` / `device_id`
  // in SYNCED data/passports.jsonl (iCloud). It MUST NOT contain the machine
  // hostname or any userAgent-derived model — that would leak the host across
  // devices. The unique part is purely RANDOM. Stability is provided by the
  // caller, which generates this once and persists it in localStorage
  // (key zeus.device.id), so it does not need to be deterministic.
  const platform = detectPlatform();
  // Prefer cryptographically-strong randomness; fall back to Math.random.
  let rand = '';
  try {
  ...
  } catch (_) { /* ignore — fall through to Math.random */ }
  if (!rand) {
