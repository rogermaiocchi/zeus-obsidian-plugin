 ...
 *
 * Cliente HTTP que fala com o daemon Swift nativo LOCAL em 127.0.0.1:2223.
 * v1.15 — ON-DEVICE puro: setBaseUrl/constructor COAGEM qualquer URL não-loopback
 * para o loopback default. O client é estruturalmente incapaz de falar com host
 * remoto — sem relay, sem Tailscale. Substitui o child_process.spawn no hot path.
 *
...
// v1.15 — ON-DEVICE puro: o transporte só fala loopback. Qualquer URL não-loopback
// (config legado, discovery, qualquer caller) é coagida para o default loopback —
// o client é estruturalmente incapaz de falar com host remoto (sem relay).
const ZEUS_LOOPBACK_DEFAULT = 'http://127.0.0.1:2223';
function _zeusCoerceLoopback(url) {
  const u = (url || '').replace(/\/$/, '');
  if (!u) return ZEUS_LOOPBACK_DEFAULT;
  if (/^https?:\/\/(127\.0\.0\.1|localhost|\[::1\]|::1)(:\d+)?(\/|$)/i.test(u)) return u;
  try { console.warn('[zeus] URL não-loopback recusada (on-device puro), usando loopback:', u); } catch (_) {}
...
class ZeusHttpClient {
  ...
  setBaseUrl(url) {
    this.baseUrl = _zeusCoerceLoopback(url);   // v1.15 — chokepoint: só loopback
    this.healthCache = null;
  }
