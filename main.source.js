...
// === v1.15 — debug-gated logging (Obsidian guideline: por padrão, só erros) ===
// console.log poluía o console do usuário a cada boot/index. Agora todo log
// informativo passa por dbg(), que só imprime quando ZEUS_DEBUG está ligado
// (settings.debug, propagado em onload). console.warn/console.error continuam
// sempre visíveis. ZEUS_DEBUG começa false — nenhum spam antes de loadSettings.
let ZEUS_DEBUG = false;
function dbg(...args) { if (ZEUS_DEBUG) { try { console.log(...args); } catch (_) {} } }
...
function _zeusFindPluginDir() {
  ...
  try {
    if (fs0 && path0) {
      const home = process.env.HOME || (process.env.USER ? '/Users/' + process.env.USER : '');
      const iCloudBase = home + '/Library/Mobile Documents/iCloud~md~obsidian/Documents';
      if (home && fs0.existsSync(iCloudBase)) {
  ...
  // 5) Fallbacks fixos macOS + iOS — derivados de $HOME (sem usernames hardcoded).
  // v1.15 — codex #8: removidos paths pessoais /Users/<user>; reconstruímos a
  // partir de process.env.HOME, que cobre qualquer usuário macOS. iOS usa o
  // sandbox determinístico /private/var/mobile.
...
const EmbedIosLib = require('./lib/embed-ios');                           // embed iOS schema versionado + embed via daemon LOCAL (on-device)
...
const DEFAULT_SETTINGS = {
  ...
  zeusDaemonUrl: 'http://127.0.0.1:2223',   // local daemon loopback (on-device puro; sem rota remota)
  ...
  deviceId: '',                             // v1.15: NÃO persistido em data.json (filtrado em saveSettings); vive em localStorage per-device
  ...
  pccMode: 'off',
  pccVisualIndicator: true,                 // exibe ☁️PCC? no status bar quando cloud era possível (heurístico)
  // v1.15 — logging gate (Obsidian guideline): default OFF mostra só warn/error.
  debug: false,                             // ON → dbg() imprime logs informativos no console
...
// v1.15 — arquitetura ON-DEVICE pura (decisão do Roger: "autonomia própria, usar as
// ferramentas do próprio OS"). Sem relay/mesh. Cada device usa o daemon Apple-nativo
// LOCAL (loopback): macOS auto-spawna bin/ZeusDaemonMac; iOS, sem daemon on-device
// (sandbox do plugin não roda código nativo), degrada para as lanes JS puras sobre os
// índices computados no Mac e sincronizados via iCloud. NENHUMA topologia de rede no
// fonte (removido o TAILSCALE_MESH hardcoded da auditoria v1.15) e nenhum peer remoto.
...
//   macOS roda seu próprio daemon nativo em 127.0.0.1:2223 (ZeusDaemonMac, auto-spawn).
//   iOS não roda daemon (sandbox) → degrada para JS puro sobre índices sincronizados.
//   O cache só persiste loopback — não há rota remota.
...
function _zeusSetLocalDaemonUrl(url) {
  try {
    ...
    if (!_zeusIsLoopback(url)) {
      dbg('[zeus] cache skip — URL não é loopback (sempre re-probe local primeiro):', url);
      return;
    }
...
//
// Ordem fixa de candidatos (ON-DEVICE puro — sem rota remota):
//   1. 127.0.0.1:2223  — daemon LOCAL Apple-nativo (ZeusDaemonMac no Mac, auto-spawn)
//   2. localhost:2223  — alternativa de hostname para o mesmo loopback
//   3. settings.zeusDaemonUrl — override (default 127.0.0.1; sincronizado via iCloud)
//
// Probes em paralelo com timeout curto (1500ms); loopback sempre vence.
async function discoverDaemonUrl(plugin, candidates = null, probeTimeoutMs = 1500) {
  ...
  if (candidates) {
    ...
    // loopback primeiro (daemon local Apple-nativo daquele device), depois o override.
    push('http://127.0.0.1:2223');
    push('http://localhost:2223');
    // override só se for loopback (on-device puro; ignora URL remota legada em data.json)
  ...
  dbg('[zeus] adaptive daemon discovery → using', chosen.url,
...
class ZeusGraphExtractor {
  ...
  async extract(filePath) {
    ...
    try {
      ...
      // v1.15 — codex round 5: passa filePath como privacyCtx para o gate central
      // ver a proveniência do conteúdo (graphExtract envia texto, não path).
      const r = await tryDaemonOrSpawn(
        this.plugin,
        'graphExtract',
...
class ZeusPassportFindModal extends obsidian.Modal {
  ...
  onOpen() {
    ...
    // v1.15 — registerDomEvent (este Modal é Component; listener limpo no close)
    this.registerDomEvent(input, 'keydown', e => { if (e.key === 'Enter') submit.click(); });
    input.focus();
  }
...
class ZeusSmartView extends ItemView {
  ...
  _renderMiniGraph(container, activeFile, neighbors) {
    ...
    for (const p of positions) {
      ...
      // v1.15 — cursor movido p/ CSS (.zeus-smart-graph-node). addEventListener
      // (não registerDomEvent) é intencional: os círculos são recriados a cada
      // render do grafo; o GC limpa o listener junto com o <circle> removido.
      // registerDomEvent acumularia uma entrada por re-render até o close da view.
...
class ZeusStatusView extends ItemView {
  ...
  async refresh() {
    ...
    // v1.15 — width é DINÂMICO (% calculado em runtime). Obsidian permite style
    // inline para valores computados; só os estáticos foram movidos p/ styles.css.
    bar.style.width = pct + '%';
    bar.setText(pct + '%');
...
class ZeusSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(containerEl)
      ...
      .setDesc('Default OFF: o console mostra só avisos e erros. ON: imprime logs informativos do Zeus (boot, indexação, discovery) para diagnóstico.')
      .addToggle(t => t.setValue(this.plugin.settings.debug === true).onChange(async v => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(`Auto-spawn no Mac quando 127.0.0.1:2223 não responde. iOS lê os índices sincronizados via iCloud (read-only) e busca nas lanes JS. Estado: ${_lcLabel}`);
    ...
    new Setting(containerEl)
      ...
      .setDesc('URL do daemon LOCAL (loopback). Mantenha em http://127.0.0.1:2223 — o plugin é on-device puro: macOS auto-spawna o ZeusDaemonMac local; iOS lê os índices sincronizados. Apenas URLs loopback (127.0.0.1/localhost/::1) são aceitas; URL remota é rejeitada (sem relay).')
      .addText(t => t.setValue(this.plugin.settings.zeusDaemonUrl).setPlaceholder('http://127.0.0.1:2223').onChange(async v => {
        // v1.15 — on-device puro: rejeita URL não-loopback (sem rota remota).
        if (v && !_zeusIsLoopback(v)) {
          new Notice('Zeus: só URLs loopback (127.0.0.1/localhost) são aceitas — arquitetura on-device, sem relay.', 8000);
    ...
    new Setting(containerEl)
      ...
      .setDesc('Limpa cache localStorage e re-prova 127.0.0.1 + settings (on-device). Loopback (daemon local Apple-nativo) sempre ganha quando responde. Use após instalar/subir o daemon local.')
    ...
    new Setting(containerEl)
      .setName('Modo PCC')
      .setDesc('off = só on-device (privacy máximo, default). opt-in = client envia header X-Zeus-Allow-Pcc:1; Apple decide caso a caso. auto = client permite PCC; o daemon apenas sinaliza POSSIBILIDADE quando a carga excede on-device — o routing real é decidido pela Apple (sem API pública de controle/inspeção).')
      .addDropdown(d => d
    ...
    new Setting(containerEl)
      ...
      .setDesc('Quando cloud (PCC) é possível, status bar exibe "☁️PCC?×N" (N = requests nesta sessão onde routing cloud era possível — heurístico, não medição real). Default ON quando pccMode ≠ off.')
    ...
    new Setting(containerEl)
      .setName('Status PCC')
      .setDesc('Inspeciona o modo atual e a contagem de requests onde cloud (PCC) era POSSÍVEL. O daemon não consegue medir uso real de cloud — Apple não expõe API de inspeção; este número é heurístico.')
      .addButton(b => b.setButtonText('Mostrar').onClick(() => {
        ...
        new Notice(
          `Zeus PCC\nmodo: ${s.mode}\núltima req com cloud possível: ${s.lastPossible ? 'sim' : 'não'}\ntotal cloud-possível nesta sessão: ${s.possibleCount}`,
          8000,
        );
      }));
    ...
    // Ações finais
    // ────────────────────────────────────────────────────────────────────
    new Setting(containerEl).setName('Ações').setHeading();
    new Setting(containerEl)
...
class ZeusPlugin extends Plugin {
  async onload() {
    ...
    trace('loadData.done');
    // v1.15 — propaga o flag de debug para o logger gated (dbg). A partir daqui,
    // dbg() respeita settings.debug; antes disso ficou silencioso (default false).
    ...
    if (isMac()) {
      ...
      try {
        ...
        // sobrescrevendo cache antigo. Sem
    ...
    // v1.15 — embed on-device: daemon local (loopback) + transformers.js labs
    ...
    this.app.workspace.onLayoutReady(async () => {
      try {
        ...
        if (!currentOk) {
          // URL atual morta — re-prova loopback + settings (on-device) em paralelo.
        ...
        if (health.status === 'ok') {
          ...
          new Notice(`Zeus: daemon ${health.platform || '?'} ${isLocal ? 'LOCAL ✓' : 'local'} · ${(health.endpoints || []).length} endpoints`);
        } else {
          // v1.4.1 — Instruções per-platform: cada device tem infra Apple nativa para rodar seu daemon.
          const macHint = 'macOS: o plugin auto-spawna bin/ZeusDaemonMac; se falhar, rode `bash daemon/scripts/install-mac-daemon.sh` (LaunchAgent).';
          const iosHint = 'iOS: sem daemon on-device (esperado) — busca roda nas lanes JS sobre os índices computados no Mac e sincronizados via iCloud.';
          const platformHint = isMac() ? macHint : iosHint;
          new Notice(`Zeus: daemon local indisponível em ${activeUrl}.\n${platformHint}`, 15000);
        }
      } catch (e) {
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (!isMac()) {
          new Notice('Zeus: Spotlight (CSSearchQuery) só roda no Mac.');
          return;
        }
        ...
        try {
          ...
          if (this.settings.audioVadEnabled) {
            ...
            if (!vad || !vad.has_speech) {
              dbg('[zeus] audio skip (no speech):', rel,
          ...
          if (!tr || !tr.text || tr.text.trim().length === 0) {
            dbg('[zeus] audio no transcript:', rel,
    ...
    // v1.15 — embed status: daemon local (on-device) + transformers.js labs
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          // Camada 1: daemon local (loopback)
      ...
      callback: async () => {
        try {
          ...
          const msg = [
            ...
            '',
            'O embedding 512-dim NLContextualEmbedding (Apple-native) é computado NO MAC',
            '(ZeusDaemonMac, auto-spawn) e persistido em data/embeddings.jsonl. Esse índice',
            'sincroniza via iCloud para o vault do iPhone/iPad.',
            '',
            'No iOS o plugin NÃO roda daemon (sandbox não executa código nativo): ele LÊ o',
            ...
            '',
            'Para reindexar: rode a indexação no Mac; o iOS recebe o índice atualizado',
            'na próxima sincronização do iCloud.',
          ].join('\n');
          dbg('[zeus.embed-ios]', msg);
          try { await navigator.clipboard.writeText(msg); } catch {}
          new Notice('Embed iOS: índice é computado no Mac e sincronizado; iOS busca via JS lanes. Instruções no clipboard.', 12000);
        } catch (e) {
  ...
  async onunload() {
    // v1.15 — Obsidian guideline: NÃO chamar detachLeavesOfType no onunload.
    // O Obsidian já gerencia o ciclo de vida das views registradas via
    // registerView(); detachar manualmente quebra "Reload plugins" e a
    // restauração de layout. Removido (era detachLeavesOfType SMART/STATUS).
    if (this.scheduler) {
  ...
  updateStatusBar(state, info) {
    ...
    if (state === 'indexing') {
      ...
      // v2.0 — PCC visual indicator (v1.15: "possível", não "usado" — heurístico)
  ...
  _zeusPromptText(promptText) {
    return new Promise((resolve) => {
      ...
      // v1.15 — elemento transitório: o modal é destruído no close e o <input>
      // (+ listener) é coletado junto. addEventListener é seguro e correto aqui.
